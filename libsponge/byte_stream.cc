...
size_t ByteStream::write(const string &data) {
    ...
    std::string tmp = data.substr(0, l); //这里string为什么要加std::
    _stream_buffer.append(BufferList(std::move(tmp))); //std::move是什么意思？
    _size += l;
    _nwritten += l;
    return l;
}
...
string ByteStream::peek_output(const size_t len) const {
    return _stream_buffer.concatenate().substr(0, min(len, _size));
}
